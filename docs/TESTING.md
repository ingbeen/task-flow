...
> AWS 배포 전, 로컬 환경에서 기능을 검증하는 체크리스트.
...
### 5-3. 에러 처리 확인 (AWS 자격 증명 없이)

```bash
# AWS 자격 증명이 설정되지 않은 환경에서 실행
./scripts/ecr-push.sh 2>&1 || true
```

- [ ] AWS 자격 증명 관련 에러 메시지가 출력되는가?
- [ ] `AWS 자격 증명을 확인할 수 없습니다`가 포함된 에러 메시지인가?

> **왜 에러인가?**
> 스크립트가 `aws sts get-caller-identity`로 Account ID를 자동 감지합니다.
> AWS CLI 자격 증명(`aws configure`)이 설정되지 않으면 이 명령이 실패하여 에러를 출력하고 종료합니다.
> 인자 없이 실행해도 AWS 자격 증명만 있으면 기본 리전(ap-northeast-2)으로 정상 동작합니다.
